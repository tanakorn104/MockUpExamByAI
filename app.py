import streamlit as st
import json
import os
from google import genai
from google.genai import types
from docx import Document # สำหรับสร้างไฟล์ Word

# ดึง API Key จาก Secrets ของ Streamlit
API_KEY = st.secrets["GEMINI_API_KEY"]
client = genai.Client(api_key=API_KEY)

# ==========================================
# 1. ฟังก์ชันโหลดค่า Config จากไฟล์ JSON
# ==========================================
def load_web_config():
    try:
        with open("web_config.json", "r", encoding="utf-8") as f:
            return json.load(f)
    except FileNotFoundError:
        # ถ้าหาไฟล์ไม่เจอ ให้ใช้ค่าเริ่มต้น
        return {
            "page_title": "Mock Exam Generator",
            "welcome_message": "กดปุ่มด้านล่างเพื่อเริ่มสร้างข้อสอบ",
            "button_text": "สร้างข้อสอบ",
            "success_message": "สำเร็จ!"
        }

config = load_web_config()
st.set_page_config(page_title=config["page_title"], page_icon="🎓")

# ==========================================
# 2. ฟังก์ชันสร้างไฟล์ Word (Export)
# ==========================================
def create_export_file(quiz_data):
    doc = Document()
    doc.add_heading('เฉลยข้อสอบ Computer Architecture', 0)
    
    for i, q in enumerate(quiz_data):
        doc.add_heading(f"ข้อที่ {i+1}: {q.get('q', '')}", level=1)
        if q.get('type') == 'choice':
            for opt in q.get('options', []):
                doc.add_paragraph(opt)
        
        doc.add_paragraph(f"คำตอบ: {q.get('a', '')}", style='Intense Quote')
        # ใช้ .get() เพื่อป้องกัน KeyError ที่เราเคยเจอกัน
        doc.add_paragraph(f"คำอธิบาย: {q.get('detail', 'ไม่มีคำอธิบายเพิ่มเติม')}")
        doc.add_paragraph("-" * 30)
        
    import io
    bio = io.BytesIO()
    doc.save(bio)
    return bio.getvalue()

# ==========================================
# 3. ฟังก์ชันเรียก AI เจนข้อสอบ
# ==========================================
def generate_quiz():
    content_path = "pre_processed_content.txt"
    instruction_path = "instruction.txt"

    try:
        # เช็คไฟล์เนื้อหาและคำสั่ง
        if not os.path.exists(content_path) or not os.path.exists(instruction_path):
            st.error("❌ ไม่พบไฟล์ข้อมูลเนื้อหาหรือคำสั่งบนระบบ (GitHub)")
            return False

        with open(content_path, "r", encoding="utf-8") as f:
            content = f.read()
        with open(instruction_path, "r", encoding="utf-8") as f:
            final_instruction = f.read()

        # ส่งข้อมูลให้ Gemini 2.5 Flash
        response = client.models.generate_content(
            model='gemini-2.5-flash',
            contents=f"เนื้อหาหลักสำหรับออกข้อสอบ:\n{content}",
            config=types.GenerateContentConfig(
                system_instruction=final_instruction,
                temperature=0.8,
                response_mime_type="application/json" # บังคับตอบเป็น JSON
            )
        )
        
        # จัดการข้อมูล JSON
        st.session_state.quiz_data = json.loads(response.text.strip())
        
        # ใส่ Default detail ป้องกัน Error
        for q in st.session_state.quiz_data:
            if 'detail' not in q:
                q['detail'] = "คำอธิบายอ้างอิงตามเนื้อหาบทเรียน"

        st.session_state.current_idx = 0
        st.session_state.app_mode = "quiz"
        return True

    except Exception as e:
        st.error(f"❌ เกิดข้อผิดพลาดในการสร้างข้อสอบ: {str(e)}")
        return False

# ==========================================
# 4. หน้าจอหลัก (UI)
# ==========================================
if "app_mode" not in st.session_state:
    st.session_state.app_mode = "start"

st.title(f"🎓 {config['page_title']}")

# --- โหมดเริ่มต้น (หน้าแรก) ---
if st.session_state.app_mode == "start":
    st.markdown(f"**{config['welcome_message']}**")
    st.info("💡 ข้อสอบจะถูกสร้างใหม่ทุกครั้งที่กดปุ่ม โดยอิงจากสไตล์ข้อสอบเก่าของอาจารย์")
    
    if st.button(config['button_text'], use_container_width=True):
        with st.spinner("🧠 AI กำลังแต่งข้อสอบระดับวิศวกรรมให้คุณ... (อาจใช้เวลา 15-30 วินาที)"):
            if generate_quiz():
                st.success(config['success_message'])
                st.rerun()

# --- โหมดทำข้อสอบ ---
elif st.session_state.app_mode == "quiz":
    st.success("✅ สร้างข้อสอบสำเร็จ! เลื่อนดูโจทย์ด้านล่างได้เลย")
    
    # วนลูปแสดงข้อสอบทั้งหมด (ให้เพื่อนๆ อ่านโจทย์แล้วคิดตาม)
    for i, q in enumerate(st.session_state.quiz_data):
        st.markdown(f"### ข้อที่ {i+1} ({q.get('type', 'unknown').upper()})")
        st.write(f"**โจทย์:** {q.get('q', '')}")
        
        if q.get('type') == 'choice':
            for opt in q.get('options', []):
                st.write(f"- {opt}")
        
        # ปุ่มกดดูเฉลยทีละข้อ
        with st.expander("👀 ดูเฉลยและคำอธิบาย"):
            st.success(f"**คำตอบ:** {q.get('a', '')}")
            st.info(f"**คำอธิบาย:** {q.get('detail', '-')}")
            
        st.divider()

    # ปุ่ม Export ไฟล์ Word
    word_file = create_export_file(st.session_state.quiz_data)
    st.download_button(
        label="💾 ดาวน์โหลดเฉลยทั้งหมดเป็นไฟล์ Word",
        data=word_file,
        file_name="ComArch_Final_MockExam.docx",
        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        use_container_width=True
    )
    
    # ปุ่มสุ่มใหม่
    if st.button("🔄 สร้างข้อสอบชุดใหม่อีกครั้ง", use_container_width=True):
        st.session_state.app_mode = "start"
        st.session_state.quiz_data = None
        st.rerun()