...
def generate_quiz():
    try:
        ...
        # สร้างค่าสุ่มจากเวลาปัจจุบัน
        current_time_seed = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")

        # ปรับปรุงคำสั่งสำหรับการสอบแบบ Open Book (เน้นการวิเคราะห์และประยุกต์ใช้)
        json_format_prompt = f"""
        คำสั่งพิเศษสำหรับการสอบแบบ Open Book (สำคัญที่สุด):
        1. จำนวนข้อสอบ: ต้องมีทั้งหมด "24 ข้อ" (ข้อ 1 ถึง 24)
        2. ภาษา: ภาษาไทย 100%
        3. ระดับความยาก: สูงมาก (Open Book Analysis Level)
        4. กฎเหล็กในการออกโจทย์:
           - "ห้าม" ถามนิยามตรงๆ (เช่น ห้ามถามว่า DMA คืออะไร) เพราะผู้สอบสามารถเปิดหนังสือตอบได้ทันที
           - โจทย์ต้องเน้น "การประยุกต์ใช้" และ "การวิเคราะห์เชิงเปรียบเทียบ" ในสถานการณ์ที่กำหนด (Scenario-based)
           - โจทย์ต้องมีการ "ผสมผสานแนวคิด" (เช่น การใช้ DMA ร่วมกับ Floating Point Calculation หรือผลกระทบของ Instruction Format ต่อ I/O throughput)
           - ต้องมีตัวเลขหรือพารามิเตอร์เฉพาะทางเทคนิคมาให้ในโจทย์ เพื่อใช้เป็นฐานในการคำนวณหรือตัดสินใจ (Seed: {current_time_seed})
        5. โครงสร้างรายการ JSON (รวม 24 รายการ):
           - ลำดับที่ 1-10 (CHOICE): สถานการณ์จำลองยาวๆ ที่มีตัวเลือกหลอกแบบสมเหตุสมผล ผู้สอบต้องวิเคราะห์พารามิเตอร์ในโจทย์ก่อนถึงจะตัดตัวเลือกได้
           - ลำดับที่ 11-20 (SHORT): โจทย์ที่สั่งให้ "วิเคราะห์ผลกระทบ", "เปรียบเทียบข้อดีข้อเสียในสถานการณ์เฉพาะ", หรือ "แสดงขั้นตอนการคำนวณแบบซับซ้อน"
           - ลำดับที่ 21-24 (SHORT Case Study): โจทย์บูรณาการขนาดใหญ่ แบ่งเป็น 'Part A: การคำนวณหรือออกแบบ' และ 'Part B: การวิจารณ์ผลลัพธ์หรือเสนอแนวทางแก้ไข'
        6. detail: ต้องอธิบาย "ตรรกะเบื้องหลัง" เฉลยอย่างละเอียด (ทำไมถึงเลือกคำตอบนี้ และทำไมตัวเลือกอื่นถึงผิดในบริบทของโจทย์)

        ตอบกลับเป็นรูปแบบ JSON Array เท่านั้น:
        [
            {{
                "type": "CHOICE",
                "q": "โจทย์สถานการณ์เชิงวิเคราะห์ภาษาไทย...",
                "options": ["...", "...", "...", "..."],
                "a": "...",
                "detail": "..."
            }}
...
if st.session_state.app_mode == "start":
    ...
    if st.button(config.get('button_text'), use_container_width=True):
        with st.spinner("AI (Gemma 3 27B) กำลังสร้างข้อสอบเชิงวิเคราะห์ระดับ Open Book 24 ข้อ..."):
            if generate_quiz():
                st.rerun()
