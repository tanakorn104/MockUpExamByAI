...
def get_config(key):
    try:
        # ลองหาใน st.secrets ก่อน (กรณีรันบน Streamlit Cloud)
        if key in st.secrets:
            return st.secrets[key]
    except Exception:
        # ดักจับ Error กรณีไม่มีไฟล์ secrets.toml ในเครื่อง (ข้ามไปเลย)
        pass
    
    # ถ้าไม่เจอใน secrets ให้หาจาก environment variable / .env
...
if not API_KEY:
    st.error("เกิดข้อผิดพลาด API KEY AI ")
else:
