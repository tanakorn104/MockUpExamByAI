...
# นำเข้า Library ตัวใหม่ของ Google
...
# --- 1. การตั้งค่า API (แบบใหม่) ---
if not API_KEY:
    st.error("เกิดข้อผิดพลาด: ไม่พบ API KEY สำหรับ AI")
    client = None
else:
    # สร้าง Client แบบใหม่แทน configure แบบเก่า
    ...
    if st.session_state.login_attempts >= 3:
        ...
        if pw:
            if pw == ADMIN_PW:
                ...
                if st.button("🚀 สร้างข้อสอบใหม่ตามเงื่อนไข") and uploaded_file and client:
                    with st.spinner("AI กำลังวิเคราะห์เนื้อหาและเจนข้อสอบ..."):
                        ...
                        try:
                            # วิธีเรียกใช้งาน AI แบบใหม่ (SDK ใหม่)
                            response = client.models.generate_content(
                                model='gemini-2.5-flash', # เปลี่ยนมาใช้โมเดลตัวใหม่ที่คุณมีสิทธิ์
                                contents=f"เนื้อหาบทเรียน: {content[:30000]}",
...
st.markdown(f"*{st.session_state.welcome_message}*") 
st.divider()
